...
# Fetch LayCTD drafts from the database
def get_layctd_drafts():
    conn = get_connection()
    with conn:
        cursor = conn.execute("SELECT * FROM layctd_drafts ORDER BY created_at DESC")
        return cursor.fetchall()

# Fetch ICF drafts from the database
def get_icf_drafts():
    conn = get_connection()
    with conn:
        cursor = conn.execute("SELECT * FROM icf_drafts ORDER BY created_at DESC")
        return cursor.fetchall()
